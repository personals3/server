...
// PUBLIC docs page — same UX as /dashboard/docs but available without
// signing in. The /api/docs listing and the /api/docs/{slug}.html pages
// are both public, so this whole experience works for strangers.
//
// We duplicate (rather than share) the dashboard variant because the
// chrome differs — this one shows a "Sign in" CTA in the top bar
...
export default function PublicDocsPage() {
  ...
  // Read ?d=slug so a permalink scrolls to the same doc.
  useEffect(() => {
  ...
  // Reflect selection back into the URL — preserves history.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const url = new URL(window.location.href);
    if (selectedSlug) url.searchParams.set("d", selectedSlug);
    else url.searchParams.delete("d");
    window.history.replaceState({}, "", url.toString());
  }, [selectedSlug]);

  useEffect(() => {
    void api<DocsListResp>("/docs", { auth: false })
      .then(setResp)
      .catch(() => setResp({ docs: [], source: "disk" }));
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const target = selected.url || `/api/docs/${selected.slug}.html`;
      ...
      <header className="sticky top-0 z-30 backdrop-blur bg-bg/80 border-b border-border">
        <div className="max-w-[1320px] mx-auto px-6 lg:px-10 h-14 flex items-center gap-3">
          ...
          <Link href="/" className="flex items-center gap-2 text-sm font-semibold">
            <Database size={15} className="text-link" />
            <span>PersonalS3</span>
            <span className="text-muted font-normal">/</span>
            <span className="text-text-soft font-normal">Docs</span>
          </Link>

          <div className="ml-auto flex items-center gap-2">
            <div className="hidden md:flex items-center gap-2 w-64 px-3 py-1.5 bg-surface border border-border rounded-full text-sm">
            ...
            <ThemeToggle />
            <Link href="/login"
              className="text-xs font-medium px-3 py-1.5 rounded-md bg-text text-bg hover:opacity-90 inline-flex items-center gap-1">
        ...
        <section className="border-b border-border">
          <div className="max-w-[1320px] mx-auto px-6 lg:px-10 py-16 lg:py-24">
            <div className="max-w-2xl">
              <p className="text-xs text-muted uppercase tracking-[0.15em] mb-4">Documentation</p>
              ...
              <p className="mt-6 text-lg text-text-soft leading-relaxed max-w-xl">
                Browse from the dashboard, script with the <code className="font-mono text-[0.9em] bg-codeBg px-1 py-0.5 rounded">ps3</code> CLI,
                or call the HTTP API directly.
              </p>
              {resp?.source === "bucket" && resp.bucket && (
                <p className="mt-6 text-xs text-muted">
                  These docs are served straight from a public PersonalS3 bucket called{" "}
                  <span className="font-mono text-text-soft">{resp.bucket}</span>.
                </p>
              )}
            </div>
          </div>
...
// ----------------------------------------------------------------------------
// Same helper components as /dashboard/docs — copied for self-containment.
// If these need to be shared later, extract into components/docs-reader.tsx.
// ----------------------------------------------------------------------------
